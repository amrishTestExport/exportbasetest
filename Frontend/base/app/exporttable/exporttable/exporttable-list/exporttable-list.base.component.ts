...
export class ExporttableListBaseComponent{
...
localStorageStateKey = "exporttable-list";
...
first: number =0;
rows: number = 0;
updatedRecords:ExporttableBase[] = [];
...
exportTableName: string ="ExportTable";
...
@ViewChild('toggleButton')
 ...
 filtersApplied:boolean = false;
  ...
  "rowHeight" : "medium"
	...
	pageViewTitle: string = 'EXPORTTABLE_LIST';
...
public location = inject(Location);
  ...
  this.clearFilterValues();
    ...
    let action: Exclude<keyof ExporttableListBaseComponent, ' '> = methodName;
 ...
 if(!(this.appUtilBaseService.isEqualIgnoreCase(value,this.filter.quickFilter, [],true))){
    ...
    return {
      ...
      ajaxUrl: ExporttableApiConstants.getDatatableData,
      select: true,
...
getParentId() {
  ...
  let property: Exclude<keyof ExporttableListBaseComponent, ' '> = value;
  if (this.isChildPage) {
    if (this[property]) {
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.exporttableService.getProtoTypingData().subscribe((data: any) => {
  ...
  this.tableSearchFieldConfig= this.appUtilBaseService.getControlsFromFormConfig(this.tableSearchConfig)
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.exporttableService.delete(requestedParams).subscribe((res: any) => {
 ...
 updateActions() {
  ...
  disablechildAction(pid?:any) {
	...
	if (this.isChildPage && this[property]) {
		...
		this.router.navigate(['../exporttabledetail'], { relativeTo: this.activatedRoute});
	}
}
