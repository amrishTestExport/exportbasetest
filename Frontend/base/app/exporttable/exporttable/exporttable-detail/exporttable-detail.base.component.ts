import { ExporttableService } from '../exporttable.service';
...
import { ExporttableApiConstants } from '@baseapp/exporttable/exporttable/exporttable.api-constants';
...
export class ExporttableDetailBaseComponent{
...
confirmationReference:any;
  ...
  "actions" : {
	...
	pageViewTitle: string = 'EXPORTTABLE_DETAIL';
	
	public exporttableService = inject(ExporttableService);
...
showMessage(config:any){
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData,actionName).subscribe(res => {
        ...
        const saveSubscription = this.exporttableService[method](requestedObj,actionName).subscribe((res:ExporttableBase) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.exporttableService.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.exporttableService.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
step:workflowInfo.step
       ...
       const methodName: any = "configureFormOnWorkflow";
        let action: Exclude<keyof ExporttableDetailBaseComponent, ' '> = methodName;
...
this.errorMessages = {
	...
	editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof ExporttableDetailBaseComponent, ' '> = fieldConfig.formulaMethod
...
addValidations(mandatoryFields:[]){
  ...
  if (environment.prototype && this.id) {
    ...
    this.exporttableService.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.exporttableService.getById (params).subscribe((res: ExporttableBase[]) => {
      this.data = res || {};
      this.formatRawData();
    });
