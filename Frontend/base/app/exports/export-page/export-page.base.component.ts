...
export class ExportPageBaseComponent  {
  ...
  initiateExport(isToastNotNeeded?: boolean) {
    ...
    if (this.fromListPage == true) {
      this.exportConfig = this.findExpConfig(this.currentImpConfig, this.fromTableName);
      const key: any = { value: this.exportConfig[0] }
      this.exportGroup.controls["templateName"].setValue(key.value.name);
    }
    let filterData: any = {};
    if (this.filters) {
