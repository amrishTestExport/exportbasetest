...
public class ExportTableBaseService<T extends ExportTableBase> extends BaseBusinessLogic<T>
		implements IExportTableBaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(ExportTableBaseService.class.getName());
	...
	@Autowired
	private  ExportTablePerimeterBaseImpl exporttablePerimeterBaseImpl;
	
	
	public ExportTableBaseService(Class<T> modelClass) {
		super(modelClass);
		...
		registerModelTransformer(PersistenceType.ANALYTICAL, new ExportTableAnalyticalTransformer());
	}
	...
	public void isObjectExists(T modelObj, boolean isUpdate) {
		T existing = getByField("name", modelObj.getName());
		
		if(isUpdate) {			
			if(existing == null || existing.getSid().equals(modelObj.getSid())) {
				return;
			}
			throw new AlreadyExistsException(ErrorCode.ALREADY_EXISTS, "ExportTable", modelObj.getName());
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("ExportTable", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("ExportTable", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("ExportTable", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	public T getByName(String name) {
		List<Filter> filters = new ArrayList<>();
		filters.add(new SimpleFilter("name", name, Operator.EQUAL));
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return exporttablePerimeterBaseImpl;
	}
}